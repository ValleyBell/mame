...
void VGMLogger::Start(void)
{
	...
	const device_image_interface* devimg = image_interface_iterator(_machine->root_device()).first();
	...
	else
	{
		...
		if (devimg != nullptr && devimg->exists())
		{
			if (devimg->longname().length() > 0)
				str2utf16(_tag.strGameNameE, devimg->longname().c_str(), 0x70);
			else
				str2utf16(_tag.strGameNameE, devimg->basename_noext(), 0x70);
			str2utf16(_tag.strAuthorNameE, devimg->manufacturer().c_str(), 0x30);
			str2utf16(_tag.strReleaseDate, devimg->year().c_str(), 0x10);
		}
	}
