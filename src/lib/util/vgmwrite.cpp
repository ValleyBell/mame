...
void VGMLogger::Start(running_machine &machine)
{
	wchar_t vgmNotes[0x50];
	
	_logging = (uint8_t)machine.options().vgm_write();
	_machine = &machine;
...
VGMDeviceLog* VGMLogger::OpenDevice(uint8_t chipType, int clock)
{
	...
	if (_logging != 0xDD)
	{
		...
		if (chipType == VGMC_ES5506)
			return nullptr;	// logging needs to be reworked for this one
		if (chipType == VGMC_SCSP)
			return nullptr;	// timing is horribly off (see Fighting Vipers)
		else if (chipType == VGMC_WSWAN)
			return nullptr;	// WonderSwan emulation is horrible in general - even the logs are screwed
	}
	...
	//		break;
		default:
			return nullptr;	// unknown chip - don't log
		}
		if (! chip_val)
		{
			vfPtr = &vf;
...
{
	if (this == nullptr || ! _vgmlog->_logging)
		return;
...
void VGMDeviceLog::FlushPCMCache(void)
{
	...
	auto& vc = *this;	// TODO: clean up
	VGMLogger::VGM_INF& vf = _vgmlog->_files[vc._fileID];
	if (! _vgmlog->_logging || vf.hFile == nullptr)
		return;
	
	VGM_PCMCACHE& vpc = vc._pcmCache;
	if (vpc.CacheData == nullptr)
		return;
	if (vpc.Start == 0xFFFFFFFF || ! vpc.Pos)
		return;
	//print_info("Flushing PCM Data: Chip %02X, Addr %06X, Size %06X\n",
	//			vc._chipType, vpc.Start, vpc.Pos);
	
	if (vpc.Pos == 0x01 && (vc._chipType & 0x7F) != VGMC_SCSP)
		singleWrt = 0x01;
	else
		singleWrt = 0x00;
	...
	if (singleWrt)
	{
		...
		fputc((vpc.Start >> 0) & 0xFF, vf.hFile);	// offset low
		fputc((vpc.Start >> 8) & 0xFF, vf.hFile);	// offset high
		fputc(vpc.CacheData[0x00], vf.hFile);		// Data
		vf.BytesWrt += 0x04;
	}
	else
	{
		...
		if (! (cmdType & 0x20))
		{
			...
			finalSize |= (vc._chipType & 0x80) << 24;
			
			fwrite(&finalSize, 0x04, 0x01, vf.hFile);	// Data Block Size
			fwrite(&vpc.Start, 0x02, 0x01, vf.hFile);		// Data Address
		}
		else
		{
			...
			fwrite(&finalSize, 0x04, 0x01, vf.hFile);	// Data Block Size
			fwrite(&vpc.Start, 0x04, 0x01, vf.hFile);		// Data Address
		}
		fwrite(vpc.CacheData, 0x01, vpc.Pos, vf.hFile);
		vf.BytesWrt += 0x07 + (finalSize & 0x7FFFFFFF);
	}
