...
void multipcm_device::device_start()
{
	...
	{
		logerror("VGM logging: getting MultiPCM '%s' ROM via address map.\n", tag());
		// As much as I hate it to hardcode the region size, I couldn't figure out how to retrieve it.
		vgm_write_large_data(m_vgm_idx, 0x01, 0x400000, 0x00, 0x00, m_direct->ptr());
	}
